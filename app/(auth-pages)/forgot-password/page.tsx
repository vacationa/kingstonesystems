    ...
    <div className="w-full flex flex-col items-center justify-center min-h-screen bg-black">
      <div className="w-full max-w-[28rem] px-4">
        <div className="text-center mb-6">
          <h1 className={`${aeonik.variable} font-aeonik text-2xl md:text-3xl font-bold text-white mb-2`}>Reset Password</h1>
          <p className={`${aeonik.variable} font-aeonik font-light text-gray-400 text-lg sm:text-xl`}>Enter your email address and we'll send you a link to reset your password.</p>
        </div>
        <Card className="border border-white/10 bg-[#111] rounded-xl text-white">
          <CardContent className="p-6">
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
              <div className="space-y-1.5">
                <Label htmlFor="email" className={`${aeonik.variable} font-aeonik text-sm text-gray-400`}>
                  Email
                </Label>
                <Input
                  {...register("email")}
                ...
                />
                {errors.email && (
                  <span className={`text-xs text-destructive ${aeonik.variable} font-aeonik`}>{errors.email.message}</span>
                )}
              </div>
