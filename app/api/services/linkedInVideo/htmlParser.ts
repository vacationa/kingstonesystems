...
class HTMLParser {
  ...
  getOgVideoUrl(secure: boolean = false) {
    const url = this.getMetaOgContent("video") || this.getMetaOgContent("video:url");
    if (secure) {
  ...
  getJsonLD<T extends object = {}>() {
    const elem = this.root.querySelector(this.genAttrSel("script", "type", "application/ld+json"));
    if (!elem) {
      return null;
    }
  ...
  genAttrSel(element: keyof JSX.IntrinsicElements, attr: string, value: string) {
