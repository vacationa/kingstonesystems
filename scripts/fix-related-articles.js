...
// Blog post data - extracted from src/pages/blog/index.tsx
const blogPosts = [
  {
...
// Create a map from slug (with or without .html) to image path
const slugToImageMap = new Map();
blogPosts.forEach(post => {
  // Map both with and without .html extension
  slugToImageMap.set(post.slug, post.image);
  slugToImageMap.set(`${post.slug}.html`, post.image);
  slugToImageMap.set(post.title, post.image);
});
...
/**
 * Get image path for a given slug or href
 */
function getImageForSlug(slugOrHref) {
  if (!slugOrHref) return null;
  
  // Try direct match
  if (slugToImageMap.has(slugOrHref)) {
    return slugToImageMap.get(slugOrHref);
  }
  
  // Extract slug from href
  const slug = extractSlugFromHref(slugOrHref);
  if (slug && slugToImageMap.has(slug)) {
    return slugToImageMap.get(slug);
  }
  
  // Try to find by partial match
  for (const [key, value] of slugToImageMap.entries()) {
...
/**
 * Fix all related articles in an HTML file
 */
function fixRelatedArticlesInFile(filePath) {
  let content = fs.readFileSync(filePath, 'utf8');
  const originalContent = content;
  
  // Only process if file has related-posts-section
  if (!content.includes('related-posts-section')) {
    return false;
  }
  
  // Extract the related posts section
  ...
  // Find all article cards with placeholders
  const articleCardPattern = /<article class="blog-post-card">([\s\S]*?)<\/article>/g;
  
  updatedSection = updatedSection.replace(articleCardPattern, (articleCard) => {
    // Check if this card has a placeholder
    if (!articleCard.includes('post-card-placeholder')) {
      return articleCard; // Already has an image, skip
    }
    
    // Extract href from the article card
    const hrefMatch = articleCard.match(/<a href="([^"]+\.html)"[^>]*>/);
    if (!hrefMatch) {
      return articleCard; // No href found, skip
    }
    
    const href = hrefMatch[1];
    const imagePath = getImageForSlug(href);
    
    if (!imagePath) {
      console.warn(`  ⚠ No image found for: ${href}`);
      return articleCard; // No image found, skip
    }
    
    // Convert absolute path to relative path (from blog/ directory)
    const relativeImagePath = imagePath.replace('/assets/', '../assets/');
    
    // Extract title for alt text
    const titleMatch = articleCard.match(/<a href="[^"]*"[^>]*>([^<]+)<\/a>/);
    const title = titleMatch ? titleMatch[1].trim() : '';
    
    // Replace placeholder with image
    const fixedCard = articleCard.replace(
      /<div class="post-card-header">\s*<div class="post-card-placeholder">[\s\S]*?<\/div>\s*<\/div>/,
      `<div class="post-card-header">
                        <img src="${relativeImagePath}" alt="${title}">
                    </div>`
    );
  ...
  if (hasChanges) {
    ...
    content = content.replace(relatedSection, updatedSection);
...
function main() {
  ...
  files.forEach(file => {
    ...
    if (fixRelatedArticlesInFile(filePath)) {
      fixedCount++;
    }
  });
